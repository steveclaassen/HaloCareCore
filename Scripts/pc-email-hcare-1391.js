...
$(function () {
    $('#dr-email-search-btn').click(function () {
        var doctorname = $('#dr-name-search').val();
        var practicenumber = $('#practice-number-search').val();
        var practicename = $('#practice-name-search').val();
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $('#dr-email-details').append('<thead><tr><th style = "width: 1%"></th><th style = "width: 12%">Practice#</th><th>Doctor name</th><th>Email address</th><th style = "width: 5%"></th><th style = "width: 1%"></th></tr></thead>');

                for (var i = 0; i < data.length; i++) {
                    var html = '';
                    //html += '<tr><td style="width: 2%"></td><td style="width: 15%">' + data[i].PracticeNumber + '</td> <td style="width: 35%">' + data[i].DoctorName + '</td > <td style="width: 35%">' + data[i].EmailAddress + '</td > <td style="width: 5%"><input type="button" class="modal-button-download" name="select-button" id="' + data[i].DoctorID + '" value="Select" onclick="select_one(this.id)"></td ><td style="width: 2%"></td></tr > ';
...
$(document).ready(function () {
    ...
    $('#emailTemplate2').on("select2:select", function (e) {
        if ($(this).val() > 0) {
            ...
            $('#validation-error-2').html('').animate({ opacity: 0 }, 500).addClass("hidden");
...
$(document).ready(function () {
    $('#email-2-send').click(function () {
        if ($('#doctor-email-to').length == 0) {
            ...
            $('#validation-email-error-2').removeClass("hidden").html('&emsp;<span style="font-size: 8px;vertical-align: middle"> <i class="fas fa-exclamation-triangle"></i></span> <b>Invalid:</b> Email address is required.').animate({ opacity: 1 }, 500);
    ...
    $('#email-2-button').click(function () {
        if ($('#emailTemplate2').val() == 0) {
            ...
            $('#validation-error-2').removeClass("hidden").html('&emsp;<span style="font-size: 8px;vertical-align: middle"> <i class="fas fa-exclamation-triangle"></i></span> <b>Invalid:</b> Please select a template from the dropdown menu.').animate({ opacity: 1 }, 500);
        }
    });
});
...
function ValidateEmail2() {
    if ($('#doctor-email-to').val() == "" || $('#doctor-email-to').val() == null) {
        ...
        $('#validation-email-error-2').html('').animate({ opacity: 0 }, 500).addClass("hidden");
    }
}
